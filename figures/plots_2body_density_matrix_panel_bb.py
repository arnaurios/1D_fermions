  ...
  # NETWORK DATA 
          if( method == "ML" ) :
              ...
              if os.path.exists(fname_ML) :
                  ...
                  v0_idx=int(20+V0)
                  dd_ml1= denmats[v0_idx,:,:]
#                  x_ml = x[v0_idx,:][:-1] + np.diff(x[v0_idx, :])/2.
#                  y_ml = y[v0_idx,:][:-1] + np.diff(y[v0_idx, :])/2.

                  if( A_num_part==2 ) : xfac=1.15;
                  if( A_num_part==3 ) : xfac=1.22;                  
                  if( A_num_part==4 ) : xfac=1.28;
                  if( A_num_part==5 ) : xfac=1.34;
                  if( A_num_part==6 ) : xfac=1.385;                
                  xfac=1.
#                  x_ml = x[v0_idx,:][:-1]/xfac + np.diff(x[v0_idx, :])/2. #/np.sqrt(2)
#                  y_ml = y[v0_idx,:][:-1]/xfac + np.diff(y[v0_idx, :])/2. #/np.sqrt(2)

                  x_ml = (x[v0_idx,:][:-1] + np.diff(x[v0_idx, :])/2.)/xfac 
                  y_ml = (y[v0_idx,:][:-1] + np.diff(y[v0_idx, :])/2.)/xfac 
